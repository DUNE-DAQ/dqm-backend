...
def create_alarm_display(name):
    ...
    #PIP-CHANGES
    #----------------------------------------------------
    def plot_heatmap(data={}, relayoutData=None):
        if not data:
            data = utils.get_last_result(partition, 'dqm0_ru', 'charge-alarm'
            cache.set(name, data, None)

        previous_data = cache.get(name)
        data = pd.concat((previous_data, pd.DataFrame(data)))

        fig = px.imshow(ndf, aspect=100, origin='lower', labels={'x': 'Test X', 'y': 'Test Y', 'color': 'Test Z'})

        #Formatting stuff, mostly copied
        fig.update_layout({'xaxis_title': 'Test X axis', 'yaxis_title': 'Test Y axis', 'title': 'Test title', 'plot_bgcolor': 'rgba(0, 0, 0, 0)', 'margin': dict(l=20, r=20, t=80, b=20)})
        fig.update_xaxes(showgrid=False, zeroline=False)
        fig.update_yaxes(showgrid=False, zeroline=False)
        fig.add_annotation(xref='paper', yref='paper', x=.9, y=1.15, text=f'Last updated at {datetime.strptime(date, "%y%m%d-%H%M%S").strftime("%H:%M:%S %d/%m/%Y")}', showarrow=False)
    ...
    #PIP-CHANGES - BUTCHERING THE LAYOUT -------------------------------------------
    layout = html.Div(
        [html.Div(children=f'Alarm display for partition {partition}', className='h1')]
        +
        ...
        +
        [html.Div(children=f'Alarm state of each channel over time', className='h1')]
        +
